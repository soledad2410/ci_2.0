...
class Login extends MY_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->user))
        {
            ...
            if ($this->input->post()){
                ...
                $message = $this->check_login($post_data);
            }
    ...
    public function check_login($data)
    {
        ...
        $query = $this->admin_model->load_admin(false, array('tbluser.user_name' => $name, 'tbluser.user_pwd' => md5($password),'tblgroup.group_level < '=>4, 'tblgroup.group_block ' => 0));
          $message = '';
        if ($query->num_rows() != 1)
        {
            $message = admin_error('Mật khẩu hoặc tài khoản đăng nhập không chính xác');
            return $message;
        } else
        {
            ...
            {
                $message = admin_error('Tài khoản của bạn đang bị tạm khóa');
                return $message;
            } else
            {
                ...
                redirect(base_url() .'admin/admin.html');
            }
        }

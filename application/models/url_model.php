...
Class Url_Model extends Base_model{
    ...
    {   
        ...
        if(isset($cate_items['menu_string']))
        {
             ...
             $url=base_url().trim($cate_items['menu_string']);
             if($page)
             {
                 $url.='_'.$page;
             }
    ...
    public function get_producttype_url($type){
        $url = '/sanpham/' . to_ansi_character($type['producttype_name']) . '_' . $type['producttype_id'] . '.html';
        return $url;
    }
    ...
    public function get_news_url($news_items,$lang = 'vi')
    {
        ...
        {
            $url=base_url().'new/'.to_ansi_character($news_items['article_title']).'_'.$news_items['article_id'].'.html';
            return $url;
        }
    }
    ...
    public function get_product_url($product_items,$lang = 'vi')
    {
        ...
        {
            $url=base_url().'pro/'.to_ansi_character($product_items['product_name']).'_'.$product_items['product_id'].'.html';
             return $url;
        }
    }
  public function emit_cate_url($cate_id,$page=false){
        ...
        if(count($cate_info)==1){
            return $this->get_cate_url($cate_info['0'],$page);
        }
     ...
     */
     public function emit_news_url($news_id){
        ...
        if(count($news_info)==1){
            ...
            $url=base_url().'new/'.to_ansi_character($news_info['0']['article_title']).'_'.$news_id.'.html';
            return $url;
        }
...
public function emit_media_url($media,$lang = vi)
{
   ...
   public function emit_product_url($product_id){
        ...
        if(count($product_info)==1){
            $url=base_url().'pro/'.to_ansi_character($product_info['0']['product_name']).'_'.$product_id.'.html';
            return $url;
        }
    }
    ...
    /**
     * Url_Model::get_album_url()
     * Deprecated to emit_album_url
     * @param mixed $album_item
     * @param string $lang
     * @return album url
     */
    public function get_album_url($album_item,$lang = 'vi')
    {
          $url=base_url().'gallery/'.$lang.'/a'.$album_item['gallery_id'].'/'.to_ansi_character($album_item['gallery_title']).'.html';
          return $url;
    }
     ...
     * @return
     */
    public function emit_page_list($cate_id=FALSE,$total,$limit,$current_page,$url=FALSE, $visible = false){
    $list='';
    $page=1;
    if($limit>0){
    ...
    if(intval($visible) > 0)
    {
        $temp = intval($visible/2);
        $start = ($current_page - $temp >0) ? $current_page - $temp : 0;
        $finish = $start + $visible;
        $finish = $finish < $page ? $finish : $page;
        if($page > 1){
        $list.='<ul class="pagination">';
        for($i=$start+1;$i<=$finish;$i++){
            if($i==$current_page){
                if($url){$list.='<li class="active"><a href="'.$url.'page='.$i.'">'.$i.'</a></li>';}else{
                $list.='<li class="active"><a href="'.$this->emit_cate_url($cate_id,$i).'">'.$i.'</a></li>';
                }
            }
            else{
...
function get_link_download($file)
{
    return $file;
    $file = base64_encode(rawurlencode($file));
    return '/home/download?file=' . $file;
}
